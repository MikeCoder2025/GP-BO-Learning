...
with tab5:
    ...
    if uploaded_file is None:
        st.info(
            ...
            Example columns:
            Temperature, Pressure, Flowrate, Conversion
            """
        )
        ...
        if len(numeric_columns) < 2:
            ...
            st.subheader("2. Define the GP / BO Problem")
            ...
            if len(input_columns) == 0:
                ...
                st.warning("Select at least one decision variable.")
                ...
                if use_constraint:
                    ...
                    if len(remaining_for_constraint) == 0:
                        ...
                        with c3:
                            default_limit = float(
                                own_data[constraint_column].median()
                            )
                ...
                st.subheader("3. Gaussian Process Settings")
                ...
                if len(model_data) < 3:
                    ...
                    st.error(
                        "At least three complete rows are required to fit the GP."
                    )
                    ...
                    X_own_scaled = own_scaler.fit_transform(X_own)
                    ...
                    st.subheader("4. Predict at Any Conditions")

                    user_values = []
                    n_ui_cols = min(3, len(input_columns))
                    ui_cols = st.columns(n_ui_cols)

                    for j, variable in enumerate(input_columns):

                        v_min = float(own_data[variable].min())
                        v_max = float(own_data[variable].max())
                        v_default = float(own_data[variable].median())

                        with ui_cols[j % n_ui_cols]:
                    ...
                    st.subheader("5. Recommend the Next Experiment")
                    ...
                    for variable in input_columns:
                        ...
                        low = float(own_data[variable].min())
                        high = float(own_data[variable].max())
                    ...
                    if len(candidate_columns) == len(input_columns):
                        ...
                        st.write("#### Recommended Operating Conditions")

                        st.dataframe(
                            recommendation.style.format(
                                {"Recommended Value": "{:.4g}"}
                            ),
                            hide_index=True,
                            use_container_width=True
                        )
                        ...
                        st.subheader("6. Add the Measured Result")
                        ...
                        if st.button(
                            ...
                            new_row_dict = {
                                variable: next_own_point[k]
                                for k, variable in enumerate(
                                    input_columns
                                )
                            }
                            ...
                            st.session_state.own_data_history.append(
                                {
                                    ...
                                    "Measured_Objective":
                                        float(measured_objective)
                                }
                            )

                            st.success(
                                "New result added. The GP will refit automatically."
                            )

                            st.rerun()

                        st.subheader("7. Download Updated Dataset")
                        ...
                        if len(
                            ...
                            st.subheader("BO Update History")
